...
function LtoRenewal() {
      ...
      if (editingId) {
        ...
        if (updateResponse.status === 200 && dataToSend.update === "Renewed") {
          ...
          const historyPayload = {
            ...
            orDate: dataToSend.orDate
              ? new Date(dataToSend.orDate)
  ...
  const handleEdit = (renewal) => {
    ...
    setFormData({
      ...
      dueDate: renewal.dueDate ? formatDate(renewal.dueDate) : "",
  ...
  const handleViewHistory = async (plateNumber) => {
    ...
    try {
      ...
      console.error(
        `Error fetching LTO history for ${plateNumber || "undefined plate"}:`,
        error
      );
      if (error.response) {
  ...
  const handleHistoryUpdate = async () => {
    ...
    if (
      !historyFormData.orNumber ||
      !historyFormData.orDate ||
      !historyFormData.renewedDate
    ) {
      // Show toast or alert
        ...
        prevData.map((record) =>
          record._id === historyFormData._id
            ? {
                ...record,
                ...payload,
                orDate: payload.orDate.toISOString(),
                renewedDate: payload.renewedDate.toISOString(),
              } // Update locally
            : record
        )
      );
  ...
  const handleAccessRequestOpen = async () => {
    ...
    try {
      ...
      const existingPendingRequest = response.data.find(req =>
        req.Module === "LTO Renewal" && // Check for LTO Renewal module
      ...
      if (existingPendingRequest) {
        toast({
          ...
          description: `You already have a pending request (${existingPendingRequest.RequestType}) for LTO Renewal access.`,
          status: "warning",
          duration: 5000,
          isClosable: true,
        });
  ...
  return (
    <Box p={4} bg="white" borderRadius="lg" boxShadow="md">
      <Flex justify="space-between" align="center" mb={4}>
        <HStack spacing={4}>
          <Text fontSize="xl" fontWeight="bold">LTO Renewals</Text>
              ...
              >
                ...
                <Td>
                  <HStack spacing={2} justify="center">
                    ...
                    <IconButton
                      icon={<TimeIcon />}
                      onClick={() => handleViewHistory(renewal.plateNumber)}
                      size="sm"
                      colorScheme="gray"
                      variant="ghost"
        ...
        <ModalContent maxW="900px">
          ...
          <ModalBody py={6}>
            {selectedRenewal && (
              <Grid templateColumns="repeat(2, 1fr)" gap={4}>
                <FormControl>
                  <FormLabel fontSize="sm" color="gray.600">Plate Number</FormLabel>
                  <Text>{selectedRenewal.plateNumber || '-'}</Text>
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" color="gray.600">Vehicle Type</FormLabel>
                  <Text>{selectedRenewal.vehicleType || '-'}</Text>
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" color="gray.600">Color</FormLabel>
                  <Text>{selectedRenewal.color || '-'}</Text>
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" color="gray.600">MVUC Rate</FormLabel>
                  <Text>{selectedRenewal.mvucRate || '-'}</Text>
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" color="gray.600">Due Date</FormLabel>
                  <Text>{selectedRenewal.dueDate ? formatDate(selectedRenewal.dueDate) : '-'}</Text>
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" color="gray.600">Registered Name</FormLabel>
                  <Text>{selectedRenewal.registeredName || '-'}</Text>
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" color="gray.600">OR Number</FormLabel>
                  <Text>{selectedRenewal.orNumber || '-'}</Text>
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" color="gray.600">OR Date</FormLabel>
                  <Text>{selectedRenewal.orDate ? formatDate(selectedRenewal.orDate) : '-'}</Text>
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" color="gray.600">Update</FormLabel>
                  <Text>{selectedRenewal.update || '-'}</Text>
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" color="gray.600">Remarks</FormLabel>
                  <Text>{selectedRenewal.remarks || '-'}</Text>
                </FormControl>
              </Grid>
            )}
          </ModalBody>
      ...
      <Modal isOpen={isAccessRequestOpen} onClose={onAccessRequestClose} isCentered>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Request Access (LTO)</ModalHeader>
