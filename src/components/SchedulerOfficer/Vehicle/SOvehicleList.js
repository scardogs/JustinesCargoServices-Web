...
const TruckTable = () => {
  ...
  const [allTrucks, setAllTrucks] = useState([]); // Add this new state for all trucks
  ...
  const [canEdit, setCanEdit] = useState(false); // State for edit permission
  const [canDelete, setCanDelete] = useState(false); // State for delete permission
  ...
  const checkAccessPermissions = async () => {
    try {
      ...
      if (!token || !userString) return; // Need user info
      
      const user = JSON.parse(userString);
      const username = user.name; // Or user.username, adjust as needed
      if (!username) return;
      ...
      const hasEditAccess = requests.some(req => 
        req.Module === "Vehicle Management" &&
        req.Username === username &&
        req.RequestType === "Edit" &&
        req.Status === "Approved" &&
        (!req.ExpiresAt || new Date(req.ExpiresAt) > now) // Check if not expired
      );

      // Check for active, approved Delete request
      const hasDeleteAccess = requests.some(req => 
        req.Module === "Vehicle Management" &&
        req.Username === username &&
      ...
      setCanDelete(hasDeleteAccess);

    } catch (error) {
      console.error("Error checking access permissions:", error);
      // Optionally show a toast, but maybe fail silently for permission checks
      setCanEdit(false); // Default to no access on error
      setCanDelete(false);
    }
  };

  // Fetch initial data and access permissions on mount
  useEffect(() => {
    fetchAllTrucks(); // Add this new function call
    fetchTrucks();
    fetchDrivers();
    fetchStubs();
    checkAccessPermissions(); // Fetch permissions on load

    // Optional: Set up an interval to periodically re-check permissions
    const intervalId = setInterval(checkAccessPermissions, 30000); // Re-check every 30 seconds
    return () => clearInterval(intervalId); // Clear interval on unmount

  }, [currentPage]); // Include dependencies if needed
  ...
  const handleChange = (e) => {
    ...
    if (["engineNo", "chassisNo", "pistonDisplacement", "noOfCylinders"].includes(name)) {
      if (/[^0-9]/.test(value)) {
  ...
  const validateField = (name, value) => {
    ...
    if (["plateNumber", "mvFileNo", "chassisNo", "engineNo", "pistonDisplacement", 
         "noOfCylinders", "fuel", "make", "series", "bodyType", "yearModel", 
  ...
  const isFieldUnique = (field, value) => {
    // Use allTrucks state for uniqueness checks
    return !allTrucks.some(
      (truck) =>
        truck[field] &&
  ...
  const validateForm = () => {
    const errors = {}; // Declare errors only once
    const {
      plateNumber,
      chassisNo,
      mvFileNo,
    ...
    const requiredFields = {
      plateNumber, chassisNo, mvFileNo, engineNo, pistonDisplacement,
      noOfCylinders, fuel, make, series, bodyType, yearModel,
      grossWeight, netWeight, shippingWeight, netCapacity, cbm,
      ownerName, ownerAddress, contactDetails, encumberedTo, status,
    };

    // Check if required fields are empty
    for (const [field, value] of Object.entries(requiredFields)) {
      if (!value || value.trim() === "") {
        // Use the existing 'errors' object
        errors[field] = `${field.charAt(0).toUpperCase() + field.slice(1).replace(/([A-Z])/g, ' $1')} is required`;
      }
    }

    // Uniqueness validation (using the existing 'plateNumber')
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const formattedData = {
        ...
        noOfCylinders: formData.noOfCylinders || "",
        fuel: formData.fuel || "",
        make: formData.make || "",
  ...
  const handleDelete = (truck) => {
    setTruckToDelete(truck); // Store the whole truck object for context if needed, or just truck._id
    onDeleteOpen();
  };

  const confirmDelete = async () => {
    if (!truckToDelete) return;
    const _id = truckToDelete._id;
    ...
    try {
      ...
      await axios.delete(
        `${process.env.NEXT_PUBLIC_BACKEND_API}/api/trucks/${_id}`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
  ...
  const handleAccessRequestOpen = async () => {
    ...
    try {
      ...
      const existingPendingRequest = response.data.find(req =>
        req.Module === "Vehicle Management" && // Check for Vehicle Management module
      ...
      if (existingPendingRequest) {
        toast({
          ...
          description: `You already have a pending request (${existingPendingRequest.RequestType}) for Vehicle Management access.`,
          status: "warning",
          duration: 5000,
          isClosable: true,
        });
  ...
  const handleSubmitRequest = async () => {
    ...
    try {
        ...
        // Generate a simple RequestID
        const generatedRequestID = `REQ-${Date.now()}-${Math.floor(Math.random() * 1000)}`;

        const payload = {
          RequestID: generatedRequestID, // Include the generated ID
          Module: "Vehicle Management", 
          RequestType: requestType, 
          Remarks: requestRemarks,
          Username: user.name, // Ensure backend expects 'Username' (capitalized)
          UserRole: user.workLevel, // Ensure backend expects 'UserRole' (capitalized)
        };

        console.log("Submitting Request Payload:", payload); // Add log to check payload
    ...
    <Box>
      ...
      <Box>
        ...
        <Box>
          ...
          >
            <InputGroup maxW="md" flexGrow={1}> {/* Allow search to grow */}
            ...
            <Flex justify="flex-end" align="center" gap={3}> {/* Use gap for spacing */}
              ...
              <Popover placement="bottom-end">
                <PopoverTrigger>
                  <IconButton
                    ...
                    // mr={3} // Remove margin, use gap in Flex
                  />
                </PopoverTrigger>
                  ...
                  <PopoverBody>
                    <VStack align="start" spacing={2}>
                      <Checkbox 
                        ...
                        onChange={(e) => setVisibleColumns({...visibleColumns, plateNumber: e.target.checked})}
                      >
                        Plate Number
                      </Checkbox>
                      <Checkbox 
                        isChecked={visibleColumns.stubNumber} 
                        onChange={(e) => setVisibleColumns({...visibleColumns, stubNumber: e.target.checked})}
                      >
                        Stub Number
                      </Checkbox>
                      <Checkbox 
                        isChecked={visibleColumns.make} 
                        onChange={(e) => setVisibleColumns({...visibleColumns, make: e.target.checked})}
                      >
                        Make
                      </Checkbox>
                      <Checkbox 
                        isChecked={visibleColumns.yearModel} 
                        onChange={(e) => setVisibleColumns({...visibleColumns, yearModel: e.target.checked})}
                      >
                        Model Year
                      </Checkbox>
                      <Checkbox 
                        isChecked={visibleColumns.bodyType} 
                        onChange={(e) => setVisibleColumns({...visibleColumns, bodyType: e.target.checked})}
                      >
                        Body Type
                      </Checkbox>
                      <Checkbox 
                        isChecked={visibleColumns.ownerName} 
                        onChange={(e) => setVisibleColumns({...visibleColumns, ownerName: e.target.checked})}
                      >
                        Owner Name
                      </Checkbox>
                      <Checkbox 
                        isChecked={visibleColumns.driverName} 
                        onChange={(e) => setVisibleColumns({...visibleColumns, driverName: e.target.checked})}
                      >
                        Driver Name
                      </Checkbox>
                      <Checkbox 
              ...
              <Tbody>
                {filteredTrucks.map((truck) => (
                  <Tr
                    ...
                    transition="all 0.2s"
                  >
                    {visibleColumns.plateNumber && <Td fontWeight="medium">{truck.plateNumber}</Td>}
                    {visibleColumns.stubNumber && <Td>{truck.stubNumber || "-"}</Td>}
                    ...
                    {visibleColumns.driverName && <Td>{truck.driverName || "-"}</Td>}
                    ...
                    <Td>
                      <HStack spacing={2} justify="center">
                        ...
                        <IconButton
                          ...
                          isDisabled={!canEdit} // Use state here
                        />
                        <IconButton
                          icon={<DeleteIcon />}
                          ...
                          isDisabled={!canDelete} // Use state here
                        />
                      </HStack>
                    </Td>
                  </Tr>
                  ...
                  >
                    ...
                    <Grid templateColumns="repeat(2, 1fr)" gap={6} mt={4}>
                      ...
                      <FormControl>
                        ...
                        <Input name="engineNo" value={formData.engineNo} onChange={handleChange} size="md" height="40px" borderColor="gray.300" _hover={{ borderColor: "blue.300" }} _focus={{ borderColor: "blue.500", boxShadow: "0 0 0 1px blue.500" }} />
                      </FormControl>
        ...
        <Modal isOpen={isDeleteOpen} onClose={onDeleteClose} isCentered>
          <ModalOverlay />
          <ModalContent>
            <ModalHeader fontWeight="bold">Confirm Deletion</ModalHeader>
            <ModalCloseButton />
            <ModalBody>
              Are you sure you want to delete truck with Plate Number: 
              <Text as="span" fontWeight="bold" ml={1}>{truckToDelete?.plateNumber}</Text>? 
        ...
        <Modal isOpen={isAccessRequestOpen} onClose={onAccessRequestClose} isCentered>
          ...
          <ModalContent>
            ...
            <ModalBody pb={6}>
              <FormControl isRequired mb={4} as="fieldset"> {/* Use fieldset for accessibility */}
                <FormLabel as="legend">Select Request Type</FormLabel> {/* Use legend */} 
                <RadioGroup onChange={setRequestType} value={requestType}>
                  <Stack direction='row' spacing={5}> {/* Use Stack for layout */} 
