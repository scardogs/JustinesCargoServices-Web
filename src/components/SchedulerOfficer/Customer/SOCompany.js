...
export default function CompaniesTable() {
  ...
  const [isModalSubmittingRequest, setIsModalSubmittingRequest] =
    useState(false);
  const [modalPendingEditRequest, setModalPendingEditRequest] = useState(false);
  const [modalPendingDeleteRequest, setModalPendingDeleteRequest] =
    useState(false);
  ...
  const checkAccessStatus = async () => {
    try {
      ...
      const approvedRequest = response.data.find(
        (request) =>
          request.Status === "Approved" && new Date(request.ExpiresAt) > now
      );
  ...
  const handleRequestAccess = async () => {
    ...
    if (requestType === "Edit" && hasEditAccess) {
      toast({
        ...
        description:
          "You already have active Edit access. Please wait for it to expire before requesting again.",
        status: "warning",
        duration: 4000,
        isClosable: true,
      });
  ...
  const handleModalRequestAccess = async () => {
    ...
    if (modalRequestType === "Edit" && modalHasEditAccess) {
      toast({
        title: "Edit Access Still Active",
        description:
          "You already have active Edit access for consignees. Please wait for it to expire before requesting again.",
        status: "warning",
        duration: 4000,
        isClosable: true,
      });
  ...
  return (
    ...
    >
      ...
      <Flex direction="column" mb={6} px={6}>
        <Flex align="center" gap={3}>
          ...
          <Tooltip
            label={
              hasAccess
                ? "You have temporary access to Edit/Delete"
                : "Request temporary access to Edit/Delete"
            }
          >
            <Button
              leftIcon={<LockIcon />}
      ...
      <Modal
        ...
        onClose={() => !isSubmittingRequest && setIsRequestModalOpen(false)}
        isCentered
      >
        <ModalOverlay />
        <ModalContent>
          ...
          <ModalBody position="relative">
            ...
            <VStack
              ...
              pointerEvents={isSubmittingRequest ? "none" : "auto"}
            >
              {isSubmittingRequest && (
              ...
              {requestType && requestRemarks && !isSubmittingRequest && (
                <Alert status="info" borderRadius="md">
                  <AlertIcon />
                  <Box>
                    <b>Summary:</b> You are requesting <b>{requestType}</b>{" "}
                    access.
                    <br />
              ...
              <FormControl isRequired>
                ...
                >
                  <option value="Edit" disabled={pendingEditRequest}>
                    Edit Access
                  </option>
                  <option value="Delete" disabled={pendingDeleteRequest}>
                    Delete Access
                  </option>
                </Select>
              </FormControl>
      ...
      <Modal
        ...
        onClose={() =>
          !isModalSubmittingRequest && setIsModalRequestOpen(false)
        }
        isCentered
      >
        <ModalOverlay />
        <ModalContent>
          ...
          <ModalBody position="relative">
            ...
            <VStack
              spacing={4}
              align="stretch"
              opacity={isModalSubmittingRequest ? 0.5 : 1}
              pointerEvents={isModalSubmittingRequest ? "none" : "auto"}
            >
              {isModalSubmittingRequest && (
                <Alert status="info" borderRadius="md">
                  <AlertIcon />
              ...
              {modalRequestType &&
                ...
                !isModalSubmittingRequest && (
                  <Alert status="info" borderRadius="md">
                    <AlertIcon />
                    <Box>
                      <b>Summary:</b> You are requesting{" "}
                      <b>{modalRequestType}</b> access.
                      <br />
              ...
              <FormControl isRequired>
                ...
                >
                  <option value="Edit" disabled={modalPendingEditRequest}>
                    Edit Access
                  </option>
                  <option value="Delete" disabled={modalPendingDeleteRequest}>
                    Delete Access
                  </option>
                </Select>
              </FormControl>
