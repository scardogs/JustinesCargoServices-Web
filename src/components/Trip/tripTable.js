...
import SetArrivalDateModal from "./setArrival";
...
const Trips = () => {
  ...
  useEffect(() => {
    ...
    // fetchTrips will be called by the other useEffect hook.
  }, []);
  ...
  useEffect(() => {
    fetchTrips();
  }, [searchTerm, searchMode, sortField, sortDirection]); // Removed currentPage, itemsPerPage
  ...
  useEffect(() => {
    ...
    // Client-side sorting.
    // For 'trip' mode, backend already sorted. Re-sorting here is optional but ensures consistency.
    // For 'waybill' mode, client-side sorting is necessary.
    if (sortField) {
