...
import { AddIcon, EditIcon, DeleteIcon, SearchIcon, ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon, CheckIcon, RepeatIcon, TriangleDownIcon } from "@chakra-ui/icons";
...
const Purchase = () => {
  ...
  const filteredSuppliers = suppliers
    .map(s => s.company)
    .filter(company => company.toLowerCase().includes(supplierInput.toLowerCase()));
  ...
  useEffect(() => {
    if (formData.warehouseID && formData.categoryID) {
      const itemsInWarehouseAndCategory = items.filter(item =>
        (item.warehouseID?._id === formData.warehouseID || item.warehouseID === formData.warehouseID) &&
        (item.categoryID?._id === formData.categoryID || item.categoryID === formData.categoryID)
      );
      setFilteredItems(itemsInWarehouseAndCategory);
    } else {
      setFilteredItems([]);
    }
  }, [formData.warehouseID, formData.categoryID, items]);
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const editGross = selectedPurchase.quantity && selectedPurchase.unitPrice ? Number(selectedPurchase.quantity) * Number(selectedPurchase.unitPrice) : 0;
      const editFinancials = computeFinancials(editGross);
      ...
      const updatedPurchase = await axios.put(
        `${process.env.NEXT_PUBLIC_BACKEND_API}/api/purchases/${selectedPurchase._id}`,
        {
          ...
          orderDate: orderDateTime,
          gross: editGross,
          netOfVat: editFinancials.netOfVat,
          vatAmount: editFinancials.vatAmount,
          ewt: editFinancials.ewt,
          paidAmount: editFinancials.paidAmount
        }
      );
  ...
  const handleAddSupplier = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `Supplier "${supplierInput}" added successfully!`,
        status: "success",
        duration: 3000,
        isClosable: true,
      });
      ...
      toast({
        title: "Error",
        description: err.response?.data?.message || "Failed to add supplier.",
        status: "error",
        duration: 3000,
        isClosable: true,
      });
  ...
  const handleDeleteSupplier = async (id, company) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Supplier "${company}" has been deleted.`,
        status: "success",
        duration: 3000,
        isClosable: true,
      });
      // If the deleted supplier is currently selected, clear it
  ...
  // For Add Purchase Modal: update dependent fields when gross changes
  useEffect(() => {
    const gross = formData.quantity && formData.unitPrice ? Number(formData.quantity) * Number(formData.unitPrice) : 0;
    const { netOfVat, vatAmount, ewt, paidAmount } = computeFinancials(gross);
    setFormData(prev => ({
  ...
  // For Edit Purchase Modal: update dependent fields when gross changes
  useEffect(() => {
    if (!isEditModalOpen || !selectedPurchase) return;
    const gross = selectedPurchase.quantity && selectedPurchase.unitPrice ? Number(selectedPurchase.quantity) * Number(selectedPurchase.unitPrice) : 0;
    const { netOfVat, vatAmount, ewt, paidAmount } = computeFinancials(gross);
  ...
  }, [selectedPurchase?.quantity, selectedPurchase?.unitPrice, isEditModalOpen]);

  // In the Update Purchase Status modal, compute proportional paid amount
  ...
  // In the Status Change Modal, add exceeded check
      ...
      <VStack spacing={6} align="stretch">
        ...
        >
          <InputGroup maxW="400px">
            ...
            <Input
              placeholder="Search by item, ID, or remarks..."
              value={searchQuery}
        ...
        >
          ...
          <Table variant="simple" fontFamily="Helvetica">
              ...
              <Tr>
                ...
                <Th isNumeric fontFamily="Helvetica">Total</Th>
            ...
            <Tbody>
                  {getCurrentPagePurchases().map((purchase) => (
                      ...
                      <Td fontFamily="Helvetica">{purchase.warehouseID?.name}</Td>
                        ...
                        <Td isNumeric fontFamily="Helvetica">₱{formatNumber(purchase.totalAmount, 2)}</Td>
                        ...
                        />
                            ...
                            <Tooltip label="Delete Purchase">
                              <IconButton
                                ...
                                isDisabled={purchase.status === "Received"}
                              />
                            </Tooltip>
                          </HStack>
                      </Td>
            ...
            <VStack spacing={6} align="stretch">
              ...
              <FormControl isRequired>
                ...
                >
                  <Icon as={FiDollarSign} color="blue.500" mr={2} />
                  <Text fontSize="xl" fontFamily="Helvetica">
                    {"₱" + formatNumber(selectedPurchase?.quantity && selectedPurchase?.unitPrice ? Number(selectedPurchase.quantity) * Number(selectedPurchase.unitPrice) : 0, 2)}
                  </Text>
                </Flex>
              </FormControl>
              ...
              <FormControl isRequired>
                <FormLabel fontWeight="medium" fontFamily="Helvetica">Net of VAT (₱)</FormLabel>
                <Input
                  value={formatNumber(selectedPurchase?.netOfVat || 0, 2)}
                  isReadOnly
                  bg="gray.50"
                  borderColor="gray.200"
                  fontWeight="bold"
              ...
              <FormControl isRequired>
                <FormLabel fontWeight="medium" fontFamily="Helvetica">VAT Amount (₱)</FormLabel>
                <Input
                  value={formatNumber(selectedPurchase?.vatAmount || 0, 2)}
                  isReadOnly
                  bg="gray.50"
                  borderColor="gray.200"
                  fontWeight="bold"
              ...
              <FormControl isRequired>
                <FormLabel fontWeight="medium" fontFamily="Helvetica">EWT (₱)</FormLabel>
                <Input
                  value={formatNumber(selectedPurchase?.ewt || 0, 2)}
                  isReadOnly
                  bg="gray.50"
                  borderColor="gray.200"
                  fontWeight="bold"
              ...
              <FormControl isRequired>
                <FormLabel fontWeight="medium" fontFamily="Helvetica">Paid Amount (₱)</FormLabel>
                <Input
                  value={formatNumber(selectedPurchase?.paidAmount || 0, 2)}
                  isReadOnly
                  bg="gray.50"
                  borderColor="gray.200"
                  fontWeight="bold"
