...
import TripDetail from "../WaybillManagement/WOTripDetail";
...
const EntityAbbreviationSummary = ({
  ...
  onViewOnlyStatusChange, // Add new callback prop
}) => {
  ...
  // Directly compute effectiveIsViewOnly for rendering
  const isDupViewOnlyBasedOnStatus =
    duplicateStatus.status === "duplicate" || duplicateStatus.viewOnly === true;
  const effectiveIsViewOnly = externalViewOnly || isDupViewOnlyBasedOnStatus;
  ...
  useEffect(() => {
    if (selectedWaybill) {
      ...
      // Reset duplicate status if no waybill is selected
      setDuplicateStatus({ status: "", reference: "", viewOnly: false });
    }
  }, [selectedWaybill]); // Only trigger on selectedWaybill change

  // Effect for notifying parent about DUPLICATE-specific view-only status changes
  useEffect(() => {
    if (onViewOnlyStatusChange) {
      onViewOnlyStatusChange(isDupViewOnlyBasedOnStatus);
    }
  }, [isDupViewOnlyBasedOnStatus, onViewOnlyStatusChange]);
  ...
  const checkDuplicateStatus = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        setDuplicateStatus({ status: "", reference: "", viewOnly: false }); // Reset on error
        return;
      }
      ...
      const {
        duplicated,
        duplicateReference,
        viewOnly: viewOnlyFromApi,
      } = await response.json();
      console.log(
        `Duplicate status for ${selectedWaybill}: ${duplicated}, reference: ${duplicateReference}, viewOnly: ${viewOnlyFromApi}`
      );

      setDuplicateStatus({
        status: duplicated,
        reference: duplicateReference,
        viewOnly: viewOnlyFromApi === true, // Ensure boolean
      });

      // No longer setting internal combined isViewOnly state here directly,
      // it's computed above and parent is notified via separate effect.
    } catch (error) {
  ...
  const handleDuplicateEntitySummaries = async () => {
    try {
      ...
      const duplicatedSummaries = sourceSummaries.map((summary) => ({
        ...summary,
        waybillNumber: selectedNewWaybill, // Use sub waybill as the new waybill number
        ...
        duplicateReference: selectedWaybill, // Reference to original
        viewOnly: true, // Add explicit view-only flag
      }));

      console.log(
        `Duplicating ${duplicatedSummaries.length} entity summaries to waybill ${selectedNewWaybill}`
      );
      console.log("Duplicated summaries:", duplicatedSummaries);

      // 4. Create new entity summaries
      if (duplicatedSummaries.length > 0) {
        await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_API}/api/entity-abbreviation-summary/waybill/${selectedNewWaybill}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify(duplicatedSummaries),
          }
        );

        // 5. Set duplicate status
        await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_API}/api/entity-abbreviation-summary/duplicate-status/${selectedNewWaybill}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
        ...
        const sourceConsigneesResponse = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_API}/api/consigneeInfo/${selectedWaybill}`
        );

        if (!sourceConsigneesResponse.ok) {
          throw new Error("Failed to fetch source consignees");
        }

        const sourceConsignees = await sourceConsigneesResponse.json();
        console.log("All source consignees:", sourceConsignees);

        // 7. Filter out consignees that have the selected sub waybill number
        const relevantConsignees = sourceConsignees.filter(
          (consignee) => consignee.subWaybillNumber === selectedNewWaybill
        );

        console.log(
          `Found ${relevantConsignees.length} consignees with subWaybillNumber ${selectedNewWaybill}`
        );
        console.log("Relevant consignees:", relevantConsignees);

        // 8. Duplicate consignee info if there are any matching the selected sub waybill
        if (relevantConsignees.length > 0) {
          const duplicatedConsignees = relevantConsignees.map((consignee) => ({
            ...consignee,
            waybillNumber: selectedNewWaybill, // Use the sub waybill number as the new waybill number
            subWaybillNumber: "", // Reset subWaybillNumber in the new record
            _id: undefined,
            duplicated: "duplicate",
            duplicateReference: selectedWaybill,
          ...
          console.log(
            `Duplicating ${duplicatedConsignees.length} consignees to waybill ${selectedNewWaybill}`
          );
          console.log("Duplicated consignees:", duplicatedConsignees);
          ...
          try {
            const response = await fetch(
              `${process.env.NEXT_PUBLIC_BACKEND_API}/api/consigneeInfo/batch`,
              {
                ...
                body: JSON.stringify(duplicatedConsignees),
              }
            );
            ...
            console.log("Batch endpoint failed, trying individual creation");
          ...
          console.log(
            `No consignees found with sub waybill number ${selectedNewWaybill}`
          );
        }
        ...
        toast({
          title: "Warning",
          description: "No entity summaries found to duplicate",
          status: "warning",
          duration: 3000,
          isClosable: true,
        });
      ...
      {selectedWaybill && (
        ...
        >
          ...
          <Flex align="center" justify="space-between" mb={4}>
            ...
            <Flex>
              {/* Only show reverse duplicate if this IS a duplicate waybill AND not externally view-only */}
              {duplicateStatus.status === "duplicate" && !externalViewOnly && (
                <Button
                  size="sm"
                  colorScheme="red"
                      ...
                      <Flex justify="space-between" mb={1}>
                        ...
                        {editingEntity === entityAbbr ? (
                          <Input
                            ...
                            type="number"
                            step="0.01"
                            isDisabled={effectiveIsViewOnly}
                          />
                        ) : (
                          <Tooltip
                            label={
                          ...
                          >
                            <Text
                              ...
                              fontWeight="semibold"
                              cursor={
                                !effectiveIsViewOnly ? "pointer" : "default"
                              }
                              onDoubleClick={() =>
                                !effectiveIsViewOnly &&
                              ...
                              _hover={{
                                textDecoration: !effectiveIsViewOnly
                                  ? "underline"
                                  : "none",
                                color: !effectiveIsViewOnly
                                  ? isRounded
                                    ? "teal.600"
                                    : isSplit
                                      ? "purple.600"
