...
const Waybill = () => {
  ...
  const handleDeleteByStub = async () => {
    ...
    try {
      ...
      if (!token) {
        toast({ title: "Error", description: "Authentication token not found. Please log in.", status: "error" });
      ...
      if (response.status === 200 && !response.data.errors) {
        toast({
          title: "Deleted",
          description: response.data.message || `All waybills for stub ${stubToDelete} deleted successfully and logged!`, 
         ...
         toast({
          ...
          description: response.data.message || `Some waybills for stub ${stubToDelete} failed to delete or log. Check logs for details.`, 
      ...
      toast({
        title: "Error",
        description: error.response?.data?.message || "Failed to delete waybills by stub.",
        status: "error",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleViewWaybills = async (stub) => { // <<< Make async
    const stubWaybills = waybills.filter((waybill) => waybill.stub === stub);
    ...
    if (waybillNumbers.length > 0) {
      try {
        ...
        toast({
          ...
          description: error.response?.data?.message || "Could not load schedule details for these waybills.",
          status: "error",
          duration: 3000,
          isClosable: true,
          position: "top-right",
  ...
  const fetchActiveTrucks = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error.response?.data?.message || "Could not load active trucks.",
        status: "error",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleConfirmReassignment = async () => {
    ...
    const selectedTruckData = activeTrucks.find(t => t.plateNumber === selectedReassignTruck);
    const selectedDriverName = selectedTruckData?.driverName || "Unknown Driver";
    ...
    console.log(`Reassigning waybill ${waybillToReassign.waybillNumber} (ID: ${waybillToReassign._id}) to truck ${selectedReassignTruck}`);
    ...
    try {
      ...
      setSelectedStubWaybills(prev => 
        prev.map(wb => wb._id === waybillToReassign._id ? updatedWaybill : wb)
      );
      ...
      toast({
        ...
        description: error.response?.data?.message || "Could not update waybill.", // Updated message
        status: "error",
        duration: 4000,
        isClosable: true,
      });
  ...
  return (
    <Box>
      ...
      <Flex mb={6} gap={4} align="center">
        <Input
          placeholder="Search stubs or vehicles..."
          value={search}
      ...
      {filteredStubs.length > 0 && (
        <Flex justify="space-between" align="center" mb={4}>
          <Text fontSize="sm" color="gray.600">
            Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, totalItems)} of {totalItems} stubs
          </Text>
          <Flex align="center">
            ...
            <Select 
              ...
              onChange={(e) => {
                ...
                const newTotalPages = Math.ceil(filteredStubs.length / newItemsPerPage);
                    ...
                    >
                      ...
                      <Td>
                        <IconButton
                          ...
                          variant="ghost"
                          onClick={() => {
                            setStubToDelete(groupedWaybill.stub); // <<< Set stub to delete
                            deleteStubAlertDisclosure.onOpen(); // <<< Open confirmation
                          }}
                          _hover={{ bg: "red.50" }}
                        />
                      </Td>
                          ...
                          <Tr key={waybill._id} _hover={{ bg: "gray.50" }}>
                            ...
                            <Td>
                              <Select
                                ...
                                onChange={(e) => handleUpdateStatusById(waybill._id, e.target.value)}
                                size="sm"
                                variant="filled"
                                bg="gray.100"
                                _hover={{ bg: "gray.200" }}
                            ...
                            <Td>
                              {isScheduleLoading ? (
                                ...
                                <VStack align="start" spacing={0}>
                                  ...
                                  <Text fontSize="xs">{waybill.reassignment.plateNumber}</Text> {/* Removed V: */} 
                                  <Text fontSize="xs">{waybill.reassignment.driverName}</Text> {/* Removed D: */} 
                                </VStack>
                              ) : relevantSchedules[waybill.waybillNumber]?.isBorrowed ? ( // <<< Keep borrowed check (This branch might be removable later)
                                  <VStack align="start" spacing={0}>
                                    <Text fontSize="xs" fontWeight="medium" color="purple.600">
                                      Borrowed By:
                                    </Text>
                                    <Text fontSize="xs">V: {relevantSchedules[waybill.waybillNumber].vehicle}</Text> {/* Keep V: for borrowed */} 
                                    <Text fontSize="xs">D: {relevantSchedules[waybill.waybillNumber].driver}</Text> {/* Keep D: for borrowed */} 
                                  </VStack>
                              ) : waybill.status === "UNUSED" ? ( // <<< Show button only if UNUSED and not reassigned/borrowed
                                <Button 
                                  size="sm" 
                                  variant="outline" 
                                  onClick={() => handleOpenReassignModal(waybill)} 
                                >
                                  Reassign
                                </Button>
                              ) : (
                                  <Text fontSize="xs" color="gray.400">N/A</Text> // <<< Indicate N/A if USED but not reassigned/borrowed
                              )}
                            </Td>
                          </Tr>
                        ))
      ...
      >
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              Are you sure you want to delete all waybills associated with stub <strong>{stubToDelete}</strong>?
              This will also remove the stub from any assigned vehicles. This action cannot be undone, but the data will be moved to the delete logs.
            </AlertDialogBody>

            <AlertDialogFooter>
              <Button ref={cancelRef} onClick={deleteStubAlertDisclosure.onClose} isDisabled={isDeletingStub}>
                Cancel
              </Button>
              <Button
                colorScheme="red"
      ...
      <Modal isOpen={isReassignModalOpen} onClose={handleCloseReassignModal} size="lg">
        <ModalOverlay />
        <ModalContent borderRadius="xl">
          ...
          <ModalCloseButton />
          <ModalBody py={5}>
            <Text mb={4}>Select an active vehicle to reassign this waybill to:</Text>
            {isFetchingActiveTrucks ? (
              ...
              <FormControl isRequired>
                ...
                <Select 
                  ...
                  size="lg"
                >
                  {activeTrucks.length === 0 && <option disabled>No active trucks found</option>}
