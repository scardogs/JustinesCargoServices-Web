...
const Calendar = ({ onSaveEvent }) => {
  ...
  const [borrowSelectedPlateNumber, setBorrowSelectedPlateNumber] =
    useState("");
  const [borrowSelectedStubNumber, setBorrowSelectedStubNumber] = useState(""); // Store stub for selected borrow vehicle
  const [allUnusedWaybills, setAllUnusedWaybills] = useState([]); // Store all for initial fetch
  ...
  // Check if a date is in the past 30 days or current date
  const isDateSelectable = (day) => {
    if (!day) return false; // If no day is provided, it's not selectable
  ...
  const fetchScheduledVehicles = async () => {
    try {
      ...
      // Then get trips to check for arrival dates
      const tripsResponse = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_API}/api/trips`
      );
      const trips = tripsResponse.data?.trips || [];

      // Group vehicles by date AND calculate active counts
      const vehiclesByDate = {};
      const activeCounts = {}; // Temporary object for counts

      schedules.forEach((schedule) => {
        const date = new Date(schedule.loadingDate);
        const dateKey = `${date.getFullYear()}-${date.getMonth()}-${date.getDate()}`;

        // Only count and add to vehiclesByDate if schedule.isOnDelivery is true
        if (schedule.isOnDelivery) {
          ...
          activeCounts[dateKey] = (activeCounts[dateKey] || 0) + 1;

          // Optional: Add a console log to verify which schedules are being counted
          console.log(
            `Calendar - Counting schedule ${schedule.scheduleID} for date ${dateKey} because isOnDelivery is true.`
          );
        } else {
          // Optional: Log why a schedule isn't counted if needed for debugging
          // console.log(`Calendar - Not counting schedule ${schedule.scheduleID} for date ${dateKey} because isOnDelivery is false.`);
        }
      });

      setScheduledVehicles(vehiclesByDate);
      setActiveScheduleCounts(activeCounts); // Set the active counts state
    } catch (error) {
      console.error("Error fetching scheduled vehicles:", error);
    }
  };

  // Fetch ALL unused waybills initially
  ...
  // Add a useEffect to reset all form values when popover closes
  useEffect(() => {
    if (popoverDay === null) {
      // Reset all form fields when popover is closed
      setPlateNumber("");
      setDriver("");
      setStubNumber("");
      setSelectedWaybill("");
      setBackLoad(false);
      setRemarks("");
      setIsBorrowed(false); // Reset the borrowed status

      // Also reset borrow-related states
      setBorrowSelectedPlateNumber("");
      setBorrowSelectedStubNumber("");
      setBorrowableWaybills([]);
      setBorrowedWaybill("");
    ...
    <Box fontFamily="Poppins, sans-serif" mt={20} ref={calendarRef}>
      ...
      <Flex flexWrap="wrap" p={2}>
        {calendarDays.map((day, index) => {
          ...
          // Get active schedule count for this day
          const dateKey = `${selectedDate.getFullYear()}-${selectedDate.getMonth()}-${day.day}`;
          const activeCount = isCurrentMonthDay
            ? activeScheduleCounts[dateKey] || 0
            : 0;

          // Get scheduled vehicles for tooltip (already filtered in fetchScheduledVehicles)
          ...
          return (
            <Tooltip
              ...
              color="black"
              boxShadow="md"
              isDisabled={!isCurrentMonthDay || popoverDay === day.day}
            >
              <Box
                onClick={() =>
                ...
                opacity={
                  !isSelectable && day.day && isCurrentMonthDay ? 0.6 : 1
                }
                position="relative"
                _hover={{
                  transform:
                    isSelectable && isCurrentMonthDay && popoverDay !== day.day
                      ? "translateY(-2px)"
                      : "none",
                  shadow:
                    isSelectable && isCurrentMonthDay && popoverDay !== day.day
              ...
              >
                ...
                <Text
                  ...
                  fontSize="sm"
                  textDecoration={
                    !isSelectable && isCurrentMonthDay && day.day
                      ? "line-through"
                      : "none"
                  }
                  mb={1}
                ...
                {day.day && isSelectable && isCurrentMonthDay && (
                  ...
                  >
                    ...
                    >
                      ...
                      <PopoverCloseButton
                        onClick={() => {
                          // Explicitly set popoverDay to null to ensure useEffect triggers
                          setPopoverDay(null);
                        }}
                      />
                      {/* Popover Header */}
                ...
                {/* Trip Count Badge - Updated to use activeCount */}
                {isCurrentMonthDay && activeCount > 0 && (
                  <Box
                    position="absolute"
                    top="3px"
                    right="3px"
