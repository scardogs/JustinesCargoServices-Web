...
const UserTable = () => {
  ...
  const [currentUserWorkLevel, setCurrentUserWorkLevel] = useState(null); // Track current user's workLevel

  useEffect(() => {
    fetchUsers();
    fetchCurrentUserWorkLevel();
  }, []);

  // Fetch the current user's workLevel
  const fetchCurrentUserWorkLevel = () => {
    const storedUser = localStorage.getItem("user");
    if (storedUser) {
      try {
        const parsedUser = JSON.parse(storedUser);
        setCurrentUserWorkLevel(parsedUser.workLevel);
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Only restrict if the user to edit is Super Admin AND current user is not a Super Admin
      if (
        currentUser &&
        currentUser.workLevel === "Super Admin" &&
        currentUserWorkLevel !== "Super Admin"
      ) {
        toast({
          title: "Forbidden",
          description: "Only Super Admin users can modify Super Admin accounts",
          status: "error",
          duration: 3000,
          isClosable: true,
          position: "top-right",
  ...
  const handleDeleteConfirm = async () => {
    ...
    // Only restrict if the user to delete is Super Admin AND current user is not a Super Admin
    if (
      userToDelete.workLevel === "Super Admin" &&
      currentUserWorkLevel !== "Super Admin"
    ) {
      toast({
        title: "Forbidden",
        description: "Only Super Admin users can delete Super Admin accounts",
        status: "error",
        duration: 3000,
        isClosable: true,
        position: "top-right",
    ...
    setIsDeleting(true); // <<< Set deleting true
    ...
    try {
      ...
      await axios.delete(
        ...
        {
          // Add headers for authentication and workLevel verification
          headers: {
      ...
      setIsDeleting(false); // <<< Set deleting false
    }
  };
  ...
  // Check if the current user can edit/delete a Super Admin account
  const canModifySuperAdmin = currentUserWorkLevel === "Super Admin";
          ...
          <Flex justify="space-between" align="center">
            <Box>
              ...
              <Heading size="lg" color="#800020">
                {
                  users.filter((user) => user?.workLevel === "Super Admin")
                    .length
                }
              </Heading>
            </Box>
              ...
              <Heading size="lg" color="#800020">
                {
                  users.filter(
                    (user) =>
                      !["Super Admin", "Admin"].includes(user?.workLevel)
                  ).length
                }
              </Heading>
            </Box>
            ...
            >
              <Tr>
                {[
                  ...
                  { name: "NAME", width: "25%", align: "left" },
                  { name: "EMAIL", width: "30%", align: "left" },
                  { name: "WORK LEVEL", width: "20%", align: "left" },
                    ...
                    <HStack spacing={2} justify="center">
                      <Tooltip
                        label={
                          user.workLevel === "Super Admin" &&
                          !canModifySuperAdmin
                            ? "Only Super Admin users can edit Super Admin accounts"
                            : "Edit user"
                        }
                        hasArrow
                      >
                      ...
                      <Tooltip
                        label={
                          user.workLevel === "Super Admin" &&
                          !canModifySuperAdmin
                            ? "Only Super Admin users can delete Super Admin accounts"
                            : "Delete user"
                        }
                        hasArrow
                      >
