...
// Move NameConfirmationDialog outside of Employee component
...
const Employee = () => {
  ...
  const handleChange = (e) => {
    ...
    // Format Government IDs
      ...
      } else if (name === 'tinNumber') { // Format: XXX-XXX-XXX (9 digits based on image)
         if (digits.length <= 3) {
          formattedValue = digits;
  ...
  // Modify handleNextStep function
  ...
  const handleUpdateStatusSubmit = async () => {
    ...
    if (!employeeToUpdateStatus || !statusFormData.status || (isDateRequired && !statusFormData.dateSeparated)) {
      toast({
        title: "Error",
        description: `Please select a status reason${isDateRequired ? ' and provide the separation date' : ''}.`, // Dynamic message
        status: "error",
        duration: 3000,
        isClosable: true,
      });
    ...
    try {
      ...
      if (statusFormData.dateSeparated) { // Only include if it has a value
          payload.dateSeparated = statusFormData.dateSeparated;
      } else if (!isDateRequired) {
          // If date is not required and not provided, explicitly send null to clear it potentially
          // Backend logic currently handles this, but being explicit can be safer
          payload.dateSeparated = null;
      }

      await axios.put(
        `${process.env.NEXT_PUBLIC_BACKEND_API}/api/employees/personal/${employeeToUpdateStatus._id}/status`,
        payload, // Send the conditional payload
        {
  ...
  return (
    <Box>
      ...
      {/* Update the NameConfirmationDialog usage */}
