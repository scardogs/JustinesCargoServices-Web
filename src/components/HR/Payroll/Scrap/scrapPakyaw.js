...
const ScrapPakyawComponent = () => {
  ...
  const handleSaveChanges = async () => {
    ...
    const token = localStorage.getItem("token");
    if (!token) { setDraftError("Authentication token not found."); setIsSavingDraft(false); return; }
  ...
  const handleCreateReport = async () => {
    if (!periodStartDate || !periodEndDate) { toast({ title: "Invalid Period", status: "error" }); return; }
    if (selectedReportRows.length === 0) { toast({ title: "No Employees Selected", status: "warning" }); return; }
    const dataForReport = payrollData.filter(r => selectedReportRows.includes(r.employeeId));
    if (dataForReport.length === 0) { toast({ title: "No Data Selected", status: "info" }); return; }

    setIsSaving(true); setError(null);
    const token = localStorage.getItem("token");
    if (!token) { setError("Authentication token not found."); setIsSaving(false); return; }

    // Get the category from the first selected row (or set to empty string)
    const category = dataForReport[0]?.category || "";
    const reportPayload = { payrollData: dataForReport, startDate: periodStartDate, endDate: periodEndDate, category };
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_API}/api/reports/payroll/scrap-pakyaw`,
        reportPayload, { headers: { Authorization: `Bearer ${token}` } }
      );
      toast({ title: "Pakyaw Report Generated", description: `Report ID: ${response.data.reportId}`, status: "success" });
      setPayrollData([]); setSelectedReportRows([]);
      if (loadDataRef.current) loadDataRef.current();
    } catch (err) {
      ...
      <Box p={4}>
        <Flex direction="column" mb={6}>
          <Flex align="center" gap={3} wrap="wrap">
            <Button bg={primaryColor} color="white" size="md" onClick={handleCreateReportIntercept} isLoading={isSaving} isDisabled={isLoading || isSavingDraft || isClearingDraft} leftIcon={<FaFileAlt />}>Create Report</Button>
            ...
            <Popover placement="bottom-start" isLazy>
              ...
              <PopoverContent zIndex={401}><PopoverArrow /><PopoverCloseButton /><PopoverHeader fontWeight="semibold">Show/Hide Columns</PopoverHeader>
                <PopoverBody><VStack align="start" spacing={1} maxHeight="250px" overflowY="auto">
