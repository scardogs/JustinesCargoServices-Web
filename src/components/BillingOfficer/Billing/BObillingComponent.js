...
import BillingPaper from "./BObillingpaper";
...
const getThirdMondayAfter = (dateString) => {
  try {
    ...
    // Find the first Monday after the invoice date
    let mondayDate = new Date(startDate);
    mondayDate.setDate(startDate.getDate() + ((8 - startDate.getDay()) % 7));

    // If the date is already a Monday, move to the next Monday
...
const BillingComponent = () => {
                        ...
                        <Portal>
                          ...
                          <MenuList minWidth="120px" zIndex="popover">
                            ...
                            <MenuItem
                              ...
                              fontSize="sm"
                              isDisabled={!canDelete || isDeletingBilling}
                            >
                              <Tooltip
                                label={
                                  deleteExpiresAt
  ...
  const StatusModal = ({ isOpen, onClose, billing, initialData, onUpdate }) => {
    ...
    const [actualAmountPaidInput, setActualAmountPaidInput] = useState(
      statusFormData.actualAmountPaid
    );
    const [chargeAmountInput, setChargeAmountInput] = useState(
      statusFormData.chargeAmount
    );
    ...
    const calculateBalance = (actualAmount, chargeAmount) => {
      ...
      if (
        statusFormData.chargeType !== "None" &&
    ...
    const handleActualAmountBlur = () => {
      setCalculatedBalance(
        calculateBalance(actualAmountPaidInput, chargeAmountInput)
      );
    };
            ...
            {statusFormData.status === "Billed" && (
              <FormControl mt={4}>
                ...
                <Input
                  ...
                  onDoubleClick={(e) =>
                    !e.currentTarget.disabled && e.currentTarget.showPicker()
                  }
                />
              </FormControl>
            )}
    ...
    try {
      ...
      if (existingPendingRequest) {
        toast({
          ...
          description: `You already have a pending request (${existingPendingRequest.RequestType}) for Billing Management access.`,
          status: "warning",
          duration: 5000,
          isClosable: true,
        });
  ...
  const checkAccessPermissions = async () => {
    try {
      ...
      setEditExpiresAt(
        editRequest?.ExpiresAt ? new Date(editRequest.ExpiresAt) : null
      );
      setDeleteExpiresAt(
        deleteRequest?.ExpiresAt ? new Date(deleteRequest.ExpiresAt) : null
      );

      // If either permission is about to expire (within 5 minutes), show a warning
      const fiveMinutesFromNow = new Date(now.getTime() + 5 * 60000);
      if (
        editRequest?.ExpiresAt &&
        new Date(editRequest.ExpiresAt) < fiveMinutesFromNow
      ) {
        toast({
          title: "Edit Access Expiring Soon",
          description: `Your edit access will expire in ${Math.ceil((new Date(editRequest.ExpiresAt) - now) / 60000)} minutes.`,
          status: "warning",
          duration: 5000,
          isClosable: true,
        });
      ...
      if (
        deleteRequest?.ExpiresAt &&
        new Date(deleteRequest.ExpiresAt) < fiveMinutesFromNow
      ) {
        toast({
          title: "Delete Access Expiring Soon",
          description: `Your delete access will expire in ${Math.ceil((new Date(deleteRequest.ExpiresAt) - now) / 60000)} minutes.`,
          status: "warning",
          duration: 5000,
          isClosable: true,
        });
  ...
  // Check permissions more frequently (every 30 seconds)
  useEffect(() => {
    ...
    <Box p={4}>
      ...
      >
        ...
        >
          ...
          <Text fontSize="2xl" fontWeight="bold" color="#550000">
            ₱{formatNumber(calculateLastWeekTotalAmountDue(billingRecords))}
          </Text>
        </Box>
        ...
        {(() => {
          ...
          const paidInLastWeek = billingRecords.filter((bill) => {
            ...
            // Use the pre-calculated previousWeek range
            return (
        ...
        >
          ...
          <Text fontSize="2xl" fontWeight="bold" color="#1a365d">
            {
              billingRecords.filter((bill) => bill.status === "Cancelled")
                .length
            }
          </Text>
        </Box>
      ...
      >
        ...
        <ModalContent>
          ...
          <ModalFooter>
            ...
            <Button
              variant="ghost"
              onClick={handleAccessRequestClose}
