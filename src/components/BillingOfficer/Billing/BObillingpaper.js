...
import WaybillBody from "../../WaybillManagement/waybillbody"; // Changed import path
...
const BillingPaper = ({
      ...
      try {
        ...
        if (entityResponseData && Array.isArray(entityResponseData)) {
          ...
          const billingMap = billingResponseData.reduce((map, billing) => {
            map[billing.billingID] = billing.storeName; // Store company name associated with billingID
            return map; 
          }, {});
            ...
            billingDetailsResponseData.forEach((detail) => {
              if (detail.waybillNumber) {
                ...
                if (companyNameForDetail) {
                  ...
                  companyBilledWaybills.get(companyNameForDetail).add(detail.waybillNumber);
                  // Also add subWaybillNumber if it exists, as it might be the primary identifier used in billing details
                  const detailSummary = entityResponseData.find(s => s.waybillNumber === detail.waybillNumber || s.subWaybillNumber === detail.waybillNumber);
                  if (detailSummary && detailSummary.subWaybillNumber) {
                    companyBilledWaybills.get(companyNameForDetail).add(detailSummary.subWaybillNumber);
                  }
                }
              }
            });
          }
          console.log("Company-specific Billed Waybills Map:", companyBilledWaybills);
          ...
          entityResponseData.forEach((summary) => {
            ...
            if (companyName) {
              ...
              const waybillsBilledByThisCompany = companyBilledWaybills.get(companyName) || new Set();
    ...
    if (
      ...
      try {
        ...
        const formattedWaybills = currentBillingDetails.map((detail, index) => {
          let description =
            detail.description || `FW: ${detail.waybillNumber}`;
  ...
  const fetchWaybillNumbers = async (company) => {
    try {
      ...
      formattedWaybills.forEach((waybill) => { // 'waybill' here is an object from the 'formattedWaybills' array
    ...
    const fetchWaybills = async () => {
      try {
        ...
        allBillingDetails.forEach((detail) => {
          ...
          // Only exclude if it's a perWaybill billing FOR THE CURRENTLY SELECTED COMPANY
          if (
            billingInfo.storeName === selectedCompany && // Ensure it's for the selected company
        ...
        // Apply the new filtering rule: exclude summaries that are split and have a subWaybillNumber
        const filteredCompanyEntitySummaries = companyEntitySummaries.filter(summary => {
          if (summary.split === "split" && summary.subWaybillNumber) {
            return false; // Exclude this summary
          }
          return true; // Keep other summaries
        });
        ...
        const initialWaybills = filteredCompanyEntitySummaries.slice(
          0,
          MAX_INITIAL_WAYBILLS
        );
        const remainingWaybills =
          filteredCompanyEntitySummaries.length > MAX_INITIAL_WAYBILLS
            ? filteredCompanyEntitySummaries.slice(MAX_INITIAL_WAYBILLS)
            : [];

        console.log(
          `Processing ${initialWaybills.length} waybills initially out of ${filteredCompanyEntitySummaries.length} total`
        );
  ...
  const refreshAllWaybills = async () => {
    if (!waybillNumbers.length) return;
    
    setIsLoading(true);
    try {
      // Refresh each waybill in sequence
      for (const waybill of waybillNumbers) {
        await refreshSingleWaybillData(waybill.waybillNumber);
      }
      // toast({
      //   title: "Success",
      //   description: "All waybills have been refreshed.",
      //   status: "success",
      //   duration: 2000,
      //   isClosable: true,
      // });
    } catch (error) {
      console.error("Error refreshing waybills:", error);
      // toast({
      //   title: "Error",
      //   description: "Failed to refresh waybills.",
      //   status: "error",
      //   duration: 3000,
      //   isClosable: true,
      // });
              ...
              <HStack spacing={4} flex={1}>
                ...
                {!isReadOnly && (
                  <Menu>
                    ...
                    >
                      {selectedDraftId
                        ? drafts.find((d) => d._id === selectedDraftId)?.draftName ||
                          "Load from Draft..."
                        : "Load from Draft..."}
                    </MenuButton>
                    <MenuList maxH="300px" overflowY="auto">
                      ...
                      {drafts.map((draft) => (
                        <MenuItem
                          key={draft._id}
                          onClick={() => handleDraftSelect({ target: { value: draft._id } })}
                          closeOnSelect={false}
                          _hover={{ bg: "gray.100" }}
                        >
                            ...
                            if (isWaybillEditMode) {
                              ...
                              if (selectedCustomer) {
                                ...
                                try {
                                  ...
                                  await handleCustomerChange({ target: { value: selectedCustomer } });
                                } catch (error) {
                                  console.error("Error refreshing waybills for edit mode:", error);
                                  toast({
                                    title: "Error",
                                    description: "Could not load all available waybills for editing.",
                                    status: "error",
                                    duration: 3000,
                                    isClosable: true,
                                  });
                                 ...
                                 console.warn("No selected customer for Edit Waybills action.");
