...
const BillingPaper = ({
    ...
    const fetchCompanies = async () => {
      try {
        ...
        if (entityResponseData && Array.isArray(entityResponseData)) {
          ...
          // Create a set of all billed waybill identifiers (could be parent or sub)
          const billedWaybillIdentifiers = new Set();
          if (billingDetailsResponseData) {
            billingDetailsResponseData.forEach((detail) => {
              // Add the waybillNumber stored in the detail (this is the unique ID used, parent or sub)
              if (detail.waybillNumber) {
                billedWaybillIdentifiers.add(detail.waybillNumber);
              }
            });
          }
          console.log("Billed Waybill Identifiers:", billedWaybillIdentifiers);
          ...
          entityResponseData.forEach((summary) => {
            ...
            if (companyName) {
              ...
              // Check if either the parent or sub waybill number is already billed
              const isBilled =
                billedWaybillIdentifiers.has(summary.waybillNumber) ||
                (summary.subWaybillNumber &&
                  billedWaybillIdentifiers.has(summary.subWaybillNumber));

              // Include if it's not billed OR if it's the company currently being edited
              if (!isBilled || isCurrentEditCompany) {
  ...
  const formatWaybillData = async (waybill, companyName) => {
    ...
    description = `Waybill: ${displayWaybillNumber}`;
  ...
  const fetchBillingDetails = async () => {
    if (
      ...
      try {
        console.log(
          "Fetching details for EDIT/READONLY (perWaybill):",
          billingData.billingID
        );
        ...
        // 1. Get billing details for *this specific* billing record
        ...
        // 2. Get all billing details for cross-reference
        const [allBillingDetailsResponse, allBillingResponse] =
          await Promise.all([
            axios.get(
              `${process.env.NEXT_PUBLIC_BACKEND_API}/api/billingDetail`
            ),
        ...
        const allBillingDetails = allBillingDetailsResponse.data || [];
        ...
        // 3. Mark waybills that are already billed elsewhere
        const billedWaybills = new Set();
        allBillingDetails.forEach((detail) => {
          const billingInfo = billingMap[detail.billingID] || {};
          // Skip waybills from current billing record and non-perWaybill types
          if (
            detail.billingID !== billingData.billingID &&
        ...
        console.log("Billed Elsewhere Waybills:", billedWaybills);
        ...
        // 5. Format waybill data for display (similar to handleCustomerChange)
        const formattedWaybills = await Promise.all(
          currentBillingDetails.map(async (detail, index) => {
            setIsLoading(true); // Keep loading state active during processing

            // Fetch shipper info to get body type - but with a 3-second timeout
            let bodyType = null;
            try {
              const shipperInfoPromise = fetchShipperInfo(detail.waybillNumber);
              const shipperInfoTimeout = new Promise((_, reject) =>
                setTimeout(
                  () => reject(new Error("Shipper info fetch timed out")),
                  3000
                )
              );

              const shipperInfo = await Promise.race([
                shipperInfoPromise,
                shipperInfoTimeout,
              ]);
              ...
              console.warn(
                `Error or timeout fetching shipper info for ${detail.waybillNumber}:`,
                error
              );
            }

            // Create description with consistent format as in add form
            let description =
              detail.description || `Waybill: ${detail.waybillNumber}`;
            if (!description.includes(detail.waybillNumber)) {
              description = `Waybill: ${detail.waybillNumber}${bodyType ? ` (${bodyType})` : ""}`;
            }
        ...
        toast({
          title: "Error",
          description:
            error.message === "Fetch operation timed out"
              ? "Loading waybills timed out. Please try again."
    ...
    try {
      ...
      const formattedWaybills = companyEntitySummaries.map((summary) => {
        const waybillDescription = summary.split
          ? `Waybill: ${summary.waybillNumber} - Split`
      ...
      // Identify waybills to exclude
      formattedWaybills.forEach((waybill) => {
        if (
          waybill.subWaybillNumber &&
          waybillMap.has(waybill.subWaybillNumber)
        ) {
          // If this waybill has a subWaybillNumber that matches another waybill's waybillNumber,
          // exclude this waybill (only render the one with matching waybillNumber)
          waybillsToExclude.add(waybill.waybillNumber);
          console.log(
            `Excluding waybill ${waybill.waybillNumber} with subWaybillNumber ${waybill.subWaybillNumber} as it matches another waybill's number`
          );
        }
      });
        ...
        relatedSummaries.forEach((summary) => {
          ...
          if (summary.subWaybillNumber) {
            const subWaybillEntry = {
              ...
              description: `Waybill: ${summary.subWaybillNumber}${waybill.description.includes(" (") ? waybill.description.substring(waybill.description.indexOf(" (")) : ""}`, // Keep only the truck type info if available
  ...
  const fetchFixRateWaybills = async (companyName) => {
    try {
      ...
      const formattedWaybills = fixRateData.map((item) => ({
        description: `Waybill: ${item.waybillNumber}`,
  ...
  const handleCustomerChange = (e) => {
    ...
    // Clear waybills and checked waybills when customer changes
    setWaybillNumbers([]);
    setCheckedWaybills({});

    if (!selectedCompany) {
      return; // Exit early if no company selected
    }

    // Make the inner function async to allow awaiting it later
    const fetchWaybills = async () => {
      try {
        ...
        // Get entity abbreviation for filtering waybills
        let companyPrefix = "";
        ...
        if (company) {
          ...
          baseAbbreviation =
            company.baseAbbreviation || company.abbreviation || "";
        } else {
          // Fallback for company abbreviation
          if (selectedCompany.toLowerCase().includes("sanford marketing")) {
            companyPrefix = "SMCO";
            baseAbbreviation = "SMCO";
          } else if (selectedCompany.toLowerCase().includes("super value")) {
            companyPrefix = "SVI";
            baseAbbreviation = "SVI";
          } else {
            companyPrefix = selectedCompany.split(" ")[0];
            baseAbbreviation = companyPrefix;
          }
        }

        console.log("Fetching waybills for company:", {
          selectedCompany,
          companyPrefix,
          baseAbbreviation,
        });

        // 1. Fetch all billed waybills to exclude them (similar approach as in fetchBillingDetails)
        ...
        // Create a set of waybill numbers already billed by any company
        const billedWaybills = new Set();
        allBillingDetails.forEach((detail) => {
          const billingInfo = billingMap[detail.billingID] || {};

          // Skip waybills from current record in edit mode
          if (isEditing && detail.billingID === billingData.billingID) {
            return;
          }

          // Only exclude if it's a perWaybill billing
          if (
            billingInfo.billingType === "perWaybill" &&
            detail.waybillNumber
          ) {
        ...
        console.log(`Already billed waybills:`, billedWaybills);
        ...
        const companyEntitySummaries = allEntitySummaries.filter((summary) => {
          ...
          // Store split/payload status
          summary.split =
            summary.split || (entityAbbr.startsWith("split-") ? "split" : "");
          summary.payload =
            summary.payload ||
            (entityAbbr.startsWith("payload-") ? "payload" : "");

          // Check company match by abbreviation
          const fullMatches =
            entityAbbr.toUpperCase() === companyPrefix.toUpperCase();
          const baseMatches =
            baseAbbr.toUpperCase() === baseAbbreviation.toUpperCase();

          // Check if not billed (unless we're editing the same record)
          let notBilled = true;

          // If parent waybill number is billed
          if (billedWaybills.has(summary.waybillNumber)) {
            notBilled = false;
          }

          // If sub-waybill number is billed
          if (
            summary.subWaybillNumber &&
            billedWaybills.has(summary.subWaybillNumber)
          ) {
            notBilled = false;
          }
        ...
        // Performance improvement: Only process the first 50 waybills initially
        const MAX_INITIAL_WAYBILLS = 50;
        const initialWaybills = companyEntitySummaries.slice(
          0,
          MAX_INITIAL_WAYBILLS
        );
        const remainingWaybills =
          companyEntitySummaries.length > MAX_INITIAL_WAYBILLS
            ? companyEntitySummaries.slice(MAX_INITIAL_WAYBILLS)
            : [];

        console.log(
          `Processing ${initialWaybills.length} waybills initially out of ${companyEntitySummaries.length} total`
        );
        if (remainingWaybills.length > 0) {
          console.log(
            `${remainingWaybills.length} waybills will be processed in the background`
          );
        }
        ...
        const uniqueWaybillNumbers = [
          ...new Set(initialWaybills.map((s) => s.waybillNumber)),
        ];
        ...
        const shipperInfoPromises = uniqueWaybillNumbers.map(async (wbNum) => {
          try {
            ...
            const timeout = new Promise((_, reject) =>
              setTimeout(() => reject(new Error("Shipper info timeout")), 3000)
            );
            ...
            console.warn(
              `Timed out or error fetching shipper info for ${wbNum}`
            );
            // Continue without shipper info
          }
        });

        try {
          await Promise.all(shipperInfoPromises);
        } catch (error) {
          console.warn(
            "Some shipper info requests failed, continuing with available data"
          );
        }
        ...
        for (const summary of initialWaybills) {
          ...
          // Determine the number to use as unique identifier
          const primaryIdentifier =
            hasSubNumber && (isSplit || isPayload)
              ? summary.subWaybillNumber
              : parentWaybillNumber;

          const displayWaybillNumber = parentWaybillNumber; // Always show parent number

          // Construct description
          let description = `Waybill: ${displayWaybillNumber}`;
          if (bodyType) {
          ...
          // Create waybill entry if not already processed
          if (!processedSubWaybills.has(primaryIdentifier)) {
            formattedWaybills.push({
              ...
              amount: summary.totalAmount
                ...
                : "0.00",
              waybillNumber: primaryIdentifier, // Use as unique identifier
        ...
        // 6. Set the formatted waybills
        setWaybillNumbers(formattedWaybills);

        // 7. In edit mode, restore checked state for existing waybills
        if (isEditing) {
          // If we have initial checked waybills saved, restore those checks
          if (
            initialCheckedRef.current &&
            Object.keys(initialCheckedRef.current).length > 0
          ) {
            setCheckedWaybills(initialCheckedRef.current);
          }
        } else {
          // Clear checks for non-edit mode
          setCheckedWaybills({});
        }

        setIsLoading(false);

        // 8. Background processing for remaining waybills if any
        if (remainingWaybills.length > 0) {
          // Process remaining waybills in the background after a short delay
          setTimeout(async () => {
            try {
              // Background fetch shipper info for remaining waybills
              const bgShipperInfoCache = new Map();
              const bgUniqueWaybillNumbers = [
                ...new Set(remainingWaybills.map((s) => s.waybillNumber)),
              ];
              ...
              for (
                let i = 0;
                i < bgUniqueWaybillNumbers.length;
                i += BATCH_SIZE
              ) {
                const batch = bgUniqueWaybillNumbers.slice(i, i + BATCH_SIZE);
                await Promise.allSettled(
                  batch.map(async (wbNum) => {
                    try {
                      const shipperInfoPromise = fetchShipperInfo(wbNum);
                      const timeout = new Promise((_, reject) =>
                        setTimeout(
                          () => reject(new Error("Shipper info timeout")),
                          2000
                        )
                      );
                      ...
                      if (info) {
                        bgShipperInfoCache.set(wbNum, info);
                      }
                    } catch (error) {
                      // Silently continue
                    }
              ...
              // Format remaining waybills
              const bgFormattedWaybills = [];
              const bgProcessedSubWaybills = new Set(processedSubWaybills); // Start with already processed

              for (const summary of remainingWaybills) {
                const parentWaybillNumber = summary.waybillNumber;
                const shipperInfo = bgShipperInfoCache.get(parentWaybillNumber);
                const bodyType = shipperInfo?.bodyType || null;
                ...
                if (bgProcessedSubWaybills.has(primaryIdentifier)) {
                  continue;
                }
                ...
                if (hasSubNumber) {
                  bgProcessedSubWaybills.add(primaryIdentifier);
                }
              }

              // Append background-loaded waybills to the existing ones
              if (bgFormattedWaybills.length > 0) {
                setWaybillNumbers((prev) => [...prev, ...bgFormattedWaybills]);
                console.log(
                  `Added ${bgFormattedWaybills.length} more waybills in background`
                );
              }
            } catch (error) {
              console.warn("Error in background waybill processing:", error);
            }
    ...
    // Fetch waybills and return the promise
    return fetchWaybills();
  };
  ...
  const generatePDF = () => {
    ...
    for (const waybill of waybillsToShow) {
      tableData.push({
        description:
          waybill.description ||
          `Waybill: ${waybill.waybillNumber || waybill.number}`,
        quantity: waybill.quantity,
        unitPrice: "",
    ...
    // Draw borders using lines
    doc.setDrawColor(220, 220, 220);
    doc.setLineWidth(0.1);
    // Then draw the borders as lines
    doc.line(totalsBoxX, totalsBoxY, totalsBoxX + totalsBoxWidth, totalsBoxY); // top
    doc.line(
      totalsBoxX + totalsBoxWidth,
      totalsBoxY,
      totalsBoxX + totalsBoxWidth,
      totalsBoxY + boxHeight
    ); // right
    doc.line(totalsBoxX, totalsBoxY, totalsBoxX, totalsBoxY + boxHeight); // left
    doc.line(
      totalsBoxX,
      totalsBoxY + boxHeight,
      totalsBoxX + totalsBoxWidth,
    ...
    addTotalRow("Total Amount Due:", rawNetAmount, true, true);
    ...
    // Calculate if there's enough space for payment section based on TABLE height
    // Re-calculate tableFinalY here for clarity
    tableFinalY =
      doc.autoTable?.previous?.finalY || yPos + estimatedContentHeight || yPos;
    if (tableFinalY + 40 > pageHeight - 30) {
      // Base check on table final Y
      // Not enough space, add a new page
      doc.addPage();
      // Reset Y position for payment section if new page is added (optional)
      // paymentY = margin; // Or adjust as needed, currently uses pageHeight - 30
    ...
    // Payment method at the bottom of the page
    const paymentY = pageHeight - 30;
    doc.setFillColor(240, 240, 240);
    doc.rect(margin, paymentY, contentWidth / 2, 8, "F");
    doc.setDrawColor(220, 220, 220);
    doc.rect(margin, paymentY, contentWidth / 2, 8, "S");
    // Payment method title
    doc.setFontSize(7);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(80, 80, 80);
    doc.text("PAYMENT METHOD:", margin + 5, paymentY + 5);
    // Draw payment methods in a horizontal layout
    const paymentX = margin + 40;
    doc.setFont("helvetica", "normal");
    doc.setTextColor(0, 0, 0);
    // CASH option
    doc.circle(paymentX, paymentY + 4, 2, "S");
    if (bottomPaymentMethod === "CASH") {
      doc.circle(paymentX, paymentY + 4, 1, "F");
    }
    doc.text("CASH", paymentX + 5, paymentY + 5);
    // CHECK option
    doc.circle(paymentX + 30, paymentY + 4, 2, "S");
    if (bottomPaymentMethod === "CHECK") {
      doc.circle(paymentX + 30, paymentY + 4, 1, "F");
    }
    doc.text("CHECK", paymentX + 35, paymentY + 5);
    // CREDIT option
    doc.circle(paymentX + 60, paymentY + 4, 2, "S");
    if (bottomPaymentMethod === "CREDIT") {
      doc.circle(paymentX + 60, paymentY + 4, 1, "F");
    }
    doc.text("CREDIT", paymentX + 65, paymentY + 5);
    // --- FOOTER ---
  ...
  const refreshSingleWaybillData = async (uniqueWaybillId) => {
    ...
    try {
      ...
      let description = `Waybill: ${parentWaybillNumber}`;
      if (bodyType) {
        description += ` (${bodyType})`;
      }
          ...
          >
            <Flex justifyContent="space-between" alignItems="center">
              <HStack spacing={4} flex={1}>
                ...
                {!isReadOnly && (
                  <Select
                    placeholder="Load from Draft..."
                    maxW="300px"
                    size="sm"
                    ml={4} // Add margin to separate from SI Number dropdown
                    focusBorderColor="#550000"
                    borderColor="#800000" // Changed border color
              ...
              <HStack mb={4} spacing={4}>
                <Box maxW="400px" flexGrow={1}>
                  ...
                  <InputGroup size="sm">
                    ...
                    <Input
                      pl="2.5rem"
                      placeholder="Waybill: 12345"
                      value={`Waybill: ${waybillSearchTerm}`}
                      onChange={(e) => {
                        const prefix = "Waybill: ";
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (isWaybillEditMode) {
                              ...
                              // Edit Waybills Logic
                              setIsWaybillEditMode(true);
                              setWaybillNumbers(allWaybillsRef.current); // Load all waybills
                            }
                          }}
