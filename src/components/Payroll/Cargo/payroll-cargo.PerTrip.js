...
import {
  ...
  PopoverCloseButton,
  PopoverFooter,
  Select, // Add Select for dropdown filter
  Divider, // Add Divider for popover separation
...
const PayrollPerTripCargo = () => { 
  ...
  // --- Get unique payment types from payrollData ---
  const paymentTypeOptions = useMemo(() => {
    const types = new Set(payrollData.map(r => r.paymentType).filter(Boolean));
    return ['All', ...Array.from(types)];
  }, [payrollData]);

  // --- ADD: State for Employee WageType Map ---
  const [employeeWageTypeMap, setEmployeeWageTypeMap] = useState({});

  // --- Fetch and cache Employee wageType mapping on data load ---
  useEffect(() => {
    const fetchEmployeeWageTypes = async () => {
      const token = localStorage.getItem("token");
      if (!token) return;
      try {
        ...
        (response.data || []).forEach(emp => {
          if (emp.empID) map[emp.empID] = emp.wageType;
        });
        setEmployeeWageTypeMap(map);
      } catch (err) {
        // Optionally handle error
      }
    };
    fetchEmployeeWageTypes();
  }, []);

  // --- Wage Type Filter State ---
  const [wageTypeFilter, setWageTypeFilter] = useState('All');
  // --- Wage Type Options (fixed list) ---
  const wageTypeOptions = ['All', 'Minimum Wage', 'Below Minimum Wage', '6 Wheel Manila', '6 Wheel Iloilo', '10 Wheel'];

  // --- UPDATE: Filtered data based on search query and wage type filter ---
  const filteredPayrollData = useMemo(() => {
    let data = payrollData;
    if (wageTypeFilter && wageTypeFilter !== 'All') {
      data = data.filter(record => {
        // Prefer wageType from record.employee, fallback to employeeWageTypeMap
        const wageType = record.employee?.wageType || employeeWageTypeMap[record.employeeId];
        return wageType === wageTypeFilter;
      });
    }
    if (!searchQuery) return data;
    // Get the rows that match the search query
    const searchMatches = data.filter(record =>
        record.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        record.employeeId.toLowerCase().includes(searchQuery.toLowerCase())
    );
    // Get the rows that are checked but don't match the search
    const checkedRows = data.filter(record => 
  ...
  }, [payrollData, searchQuery, selectedReportRows, wageTypeFilter, employeeWageTypeMap]);
  ...
  // --- ADD: State for Hide Columns Modal ---
  const { isOpen: isHideColumnsOpen, onOpen: onHideColumnsOpen, onClose: onHideColumnsClose } = useDisclosure();
    ...
    <MotionBox>
      <Box p={4}>
        ...
        <Flex justify="space-between" align="center" mb={6} wrap="wrap">
          {/* Left side: Period Covered, Search, and Change Date */}
          ...
          {/* Right side: Filter and Settings - Pushed to the far right */}
          <HStack spacing={3} wrap="wrap" ml="auto">
            {/* --- Wage Type Dropdown Filter --- */}
            <Select
              width="180px"
              size="md"
              ...
              bg="white"
            >
              {wageTypeOptions.map(type => (
                <option key={type} value={type}>{type}</option>
              ))}
            </Select>
            {/* --- END: Wage Type Dropdown Filter --- */}
            <Popover placement="bottom-end" isLazy>
              ...
              <PopoverContent zIndex={401} minW="220px"> {/* Ensure it's above table header */}
                <PopoverArrow />
                <PopoverCloseButton />
        ...
        {/* --- Hide Columns Modal --- */}
        <Modal isOpen={isHideColumnsOpen} onClose={onHideColumnsClose} isCentered>
          <ModalOverlay />
          <ModalContent>
            ...
            <ModalFooter>
              <Button onClick={onHideColumnsClose} colorScheme="blue">
                Close
              </Button>
            </ModalFooter>
          </ModalContent>
        ...
        <ModalContent borderRadius="lg"> 
          ...
          <ModalBody py={5} px={6}> 
            ...
            >
                    ...
                    <Thead bg={secondaryColor}>
                      <Tr>
                        <Th color="white" textAlign="center">
                          <Checkbox
                            isChecked={selectedWaybills.length === fetchedTripDetails.length && fetchedTripDetails.length > 0}
                            isIndeterminate={selectedWaybills.length > 0 && selectedWaybills.length < fetchedTripDetails.length}
                            onChange={handleSelectAllWaybills}
                            size="lg" // Changed from 'sm' to 'lg' for bigger checkbox
                            colorScheme="blue"
                            sx={{ '.chakra-checkbox__control': { width: '1.5em', height: '1.5em' } }} // Make box itself larger
                          />
                        </Th>
                    ...
                    <Tbody>
                      ...
                      {(() => {
                        ...
                        return Object.entries(tripGroups).map(([tripID, waybills]) => {
                          // Determine if all waybills in this trip are checked
                          const allChecked = waybills.every(wb => selectedWaybills.includes(wb.waybillNumber));
                          const someChecked = waybills.some(wb => selectedWaybills.includes(wb.waybillNumber));
                          const handleTripCheckbox = (e) => {
                            if (e.target.checked) {
                              // Add all waybills in this trip
                              setSelectedWaybills(prev => Array.from(new Set([...prev, ...waybills.map(wb => wb.waybillNumber)])));
                            } else {
                              // Remove all waybills in this trip
                              setSelectedWaybills(prev => prev.filter(wbNum => !waybills.some(wb => wb.waybillNumber === wbNum)));
                            }
                          };
              ...
              {/* --- OVERALL TOTAL RATE & TOTAL TRIPS --- */}
              <Box mt={4} p={3} bg={"gray.50"} borderRadius="md" borderWidth="1px" borderColor={"gray.200"} textAlign="right">
                <Text fontWeight="bold" fontSize="lg" color={primaryColor}>
                  Overall Total Rate: {pesoFormatter.format(selectedWaybills.reduce((sum, waybillNumber) => sum + (parseFloat(editedTripRates[waybillNumber]) || 0), 0))}
                </Text>
                <Text fontWeight="semibold" fontSize="md" color={secondaryColor} mt={1}>
                  Total Trips: {selectedWaybills.length}
                </Text>
              </Box>
            </Box>
          </ModalBody>
