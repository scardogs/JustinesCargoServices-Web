...
const PayrollDailyCargo = () => { // Renamed component for specificity
  ...
  const earningsColBg = "#E0F7FA"; // Lighter Blue for earnings columns (adjust if needed)
  ...
  const debouncedSaveDraft = useCallback(
    debounce(async (dataToSave, token, currentPeriodStart, currentPeriodEnd) => {
      ...
      // Ensure all items have correct period dates (safety check)
      const draftsWithDates = dataToSave.map(d => ({
          ...d,
          startDate: currentPeriodStart,
          endDate: currentPeriodEnd
      }));

      console.log("Auto-saving drafts for period:", currentPeriodStart, "-", currentPeriodEnd);
      try {
        await axios.put(
          `${process.env.NEXT_PUBLIC_BACKEND_API}/api/payroll/daily-cargo/bulk-upsert`,
          draftsWithDates, // Send the array of drafts
          { headers: { Authorization: `Bearer ${token}` } }
        );
        console.log("Drafts auto-saved successfully.");
        setDraftError(null); // Clear draft error on success
      } catch (err) {
        console.error("Auto-save draft error:", err);
        const errorMsg = err.response?.data?.message || err.message || "Auto-save failed.";
        setDraftError(errorMsg); // Set draft error state
      }
    }, 1500), // Debounce time in ms (e.g., 1.5 seconds)
    [] // Dependencies for useCallback
  );

  // Handle field update - Trigger auto-save
  const handleFieldUpdate = (employeeId, field, value) => {
    ...
    // Call debounced save AFTER the state update is likely processed
    const token = localStorage.getItem("token");
    // Pass the 'updatedData' directly, as state update might be async
      ...
      if (fetchedDrafts.length > 0) {
          ...
          try {
              ...
              const dailyEmployees = (employeeResponse.data || []).filter(
                  (emp) => emp.salaryCategory === "Daily" && emp.dateSeparated === null && emp.Department === "Justine's Scrap"
              );
  ...
  const handleCreateReport = async () => {
    ...
    try {
      const response = await axios.post(
        ...
        `${process.env.NEXT_PUBLIC_BACKEND_API}/api/reports/payroll/scrap-daily`,
        reportPayload,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast({
            ...
            <Table variant="simple" size="sm" className="payroll-table">
              ...
              <Thead position="sticky" top={0} zIndex={1} bg={headerBg} boxShadow="sm">
                 ...
                 <Tr>
                    ...
                    {/* --- UPDATED: Earnings Group Header with dynamic colSpan --- */}
                    ...
                    {/* --- UPDATED: Deductions Group Header with dynamic colSpan --- */}
                    {visibleDeductionsColumns > 0 && <Th colSpan={visibleDeductionsColumns} borderBottomWidth="2px"  textAlign="center" bg={deductionsHeaderBg}>DEDUCTIONS</Th>}
                 ...
                 <Tr>
                    {/* --- ADDED: TH with Select All Checkbox --- */}
                    <Th borderBottomWidth="2px" borderColor={secondaryColor} textAlign="center" verticalAlign="middle" px={1}>
                       <Checkbox
                           size="lg"
                           sx={{
                           ...
                           isChecked={
                               ...
                               selectedReportRows.length === filteredPayrollData.length
                           }
                           isIndeterminate={
                    ...
                    {/* Added 4 empty TH cells to align under Period Covered */}
                    <Th borderBottomWidth="2px" borderColor={secondaryColor} textAlign="center" verticalAlign="middle"></Th>
                    <Th borderBottomWidth="2px" borderColor={secondaryColor} textAlign="center" verticalAlign="middle"></Th>
                    <Th borderBottomWidth="2px" borderColor={secondaryColor} textAlign="center" verticalAlign="middle"></Th>
              ...
              {/* === TBODY (Added Conditional Rendering) === */}
              <Tbody>
                  {/* --- UPDATED: colSpan for Loading/Error/No Data rows --- */}
                  {isLoading ? (
                      ...
                      filteredPayrollData.map((record, index) => (
                          <MotionTr key={record.employeeId || index} _hover={{ bg: 'gray.50' }}>
                              {/* --- ADDED: Checkbox Cell --- */} 
                              ...
                              {columnVisibility.dailyWage && <Td px={2} isNumeric fontSize="xs"> <NumberInput size="xs" value={record.dailyWage} isReadOnly precision={2}> <NumberInputField px={1} border="none" cursor="default"/> </NumberInput> </Td>}
                              {columnVisibility.regularDaysWorked && <Td px={1} isNumeric fontSize="xs"> <NumberInput size="xs" value={record.regularDaysWorked} onChange={(v) => handleFieldUpdate(record.employeeId, 'regularDaysWorked', v)} min={0} precision={1}> <NumberInputField px={1} /> </NumberInput> </Td>}
                              {/* NEW: Basic Pay Cell (Calculated) */} 
                              ...
                              {/* Earnings Adjustment */} 
                              {columnVisibility.earningsAdjustment && <Td px={1} isNumeric fontSize="xs" bg={earningsColBg}> <NumberInput size="xs" value={record.earningsAdjustment} onChange={(v) => handleFieldUpdate(record.employeeId, 'earningsAdjustment', v)} precision={2}> <NumberInputField px={1} bg="transparent"/> </NumberInput> </Td>}
                              {/* Over Time */} 
                              {columnVisibility.overTime && <Td px={1} isNumeric fontSize="xs" bg={earningsColBg}> <NumberInput size="xs" value={record.overTime} onChange={(v) => handleFieldUpdate(record.employeeId, 'overTime', v)} min={0} precision={2}> <NumberInputField px={1} bg="transparent"/> </NumberInput> </Td>}
                              {/* Holiday Pay */} 
                              {columnVisibility.holidayPay && <Td px={1} isNumeric fontSize="xs" bg={earningsColBg}> <NumberInput size="xs" value={record.holidayPay} onChange={(v) => handleFieldUpdate(record.employeeId, 'holidayPay', v)} min={0} precision={2}> <NumberInputField px={1} bg="transparent"/> </NumberInput> </Td>}
                              {/* SIL Pay */} 
                              {columnVisibility.silPay && <Td px={1} isNumeric fontSize="xs" bg={earningsColBg}>
                                 <NumberInput size="xs" value={record.silPay} isReadOnly precision={2}>
                                    <NumberInputField 
                                        px={1} 
                                        bg="transparent" 
                                        border="none" 
                                ...
                                <Td px={1} isNumeric fontSize="xs" bg={earningsColBg} verticalAlign="middle">
                                  ...
                                  {(!record.thirteenthMonth || parseFloat(record.thirteenthMonth) === 0) ? (
                                    <Button
                                      ...
                                      isDisabled={applying13thMonth && applying13thMonth[record.employeeId]}
                                      colorScheme="blue"
                                      variant="outline"
                                    >
                                      Apply
                                    ...
                                    <NumberInput
                                      ...
                                      onChange={(v) => handleFieldUpdate(record.employeeId, 'thirteenthMonth', v)}
                                      min={0}
                                      precision={2}
                                    >
                                      <NumberInputField
                              ...
                              {columnVisibility.deductionsAdjustment && <Td px={1} isNumeric fontSize="xs" bg={deductionsColBg}> <NumberInput size="xs" value={record.deductionsAdjustment} onChange={(v) => handleFieldUpdate(record.employeeId, 'deductionsAdjustment', v)} precision={2}> <NumberInputField px={1} bg="transparent"/> </NumberInput> </Td>}
                              {/* SSS */} 
                              {columnVisibility.sss && <Td px={1} isNumeric fontSize="xs" bg={deductionsColBg}>
                                 <Editable
                                    ...
                                    isDisabled={!record.isOverride}
                                    onSubmit={(nextValue) => handleFieldUpdate(record.employeeId, 'sss', nextValue)}
                                    fontSize="xs" 
                                    w="full"
                                 >
                                     <Tooltip label={record.isOverride ? "Manual Entry (Override ON)" : `Calculated based on Gross Pay: ${pesoFormatter.format(record.totalGrossPay)}`} placement="top">
                                        ...
                                        px={1} 
                                            py={0.5}
                                            w="full"
                                            _hover={{ bg: record.isOverride ? 'gray.100' : 'transparent'}}
                                            cursor={record.isOverride ? "text" : "default"}
                                    />
                                     </Tooltip>
                                     <EditableInput 
                                        type="number"
                              ...
                              {columnVisibility.philhealth && <Td px={1} isNumeric fontSize="xs" bg={deductionsColBg}>
                                <Editable
                                    ...
                                    isPreviewFocusable={true}
                                    isDisabled={!record.isOverride}
                                    onSubmit={(nextValue) => handleFieldUpdate(record.employeeId, 'philhealth', nextValue)}
                                    fontSize="xs" 
                                    w="full"
                                 >
                                     <Tooltip label={record.isOverride ? "Manual Entry (Override ON)" : `Calculated based on Daily Wage: ${pesoFormatter.format(record.dailyWage)}`} placement="top">
                                         <EditablePreview px={1} py={0.5} w="full" _hover={{ bg: record.isOverride ? 'gray.100' : 'transparent'}} cursor={record.isOverride ? "text" : "default"}/>
                                     </Tooltip>
                                     <EditableInput type="number" step="0.01" min="0" px={1} py={0.5} size="xs" textAlign="right"/>
                                 </Editable>
                               </Td>}
                              {/* Pag-IBIG */} 
                              {columnVisibility.pagibig && <Td px={1} isNumeric fontSize="xs" bg={deductionsColBg}>
                                 <Editable
                                    ...
                                    isPreviewFocusable={true}
                                    isDisabled={!record.isOverride}
                                    onSubmit={(nextValue) => handleFieldUpdate(record.employeeId, 'pagibig', nextValue)}
                                    fontSize="xs" 
                                    w="full"
                                 >
                              ...
                              {columnVisibility.caCharges && <Td px={1} isNumeric fontSize="xs" bg={deductionsColBg}>
                                  <Editable
                                    ...
                                    isPreviewFocusable={true}
                                    isDisabled={!record.isOverride}
                                    onSubmit={(nextValue) => handleFieldUpdate(record.employeeId, 'caCharges', nextValue)}
                                    fontSize="xs" 
                                    w="full"
                                 >
                              ...
                              {/* W/holding Tax */} 
                              {columnVisibility.withholdingTax && <Td px={1} isNumeric fontSize="xs" bg={deductionsColBg}> <NumberInput size="xs" value={record.withholdingTax} onChange={(v) => handleFieldUpdate(record.employeeId, 'withholdingTax', v)} min={0} precision={2}> <NumberInputField px={1} bg="transparent"/> </NumberInput> </Td>}
                              {/* Total Deductions */} 
               ...
               {!isLoading && !error && filteredPayrollData.length > 0 && (
                   <Tfoot bg="gray.100">
                       <Tr>
                           ...
                           <Th colSpan={9} textAlign="right" py={2}>TOTALS:</Th> 
