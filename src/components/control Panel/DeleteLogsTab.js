...
import { FiSearch, FiEye, FiTrash2, FiRotateCcw, FiChevronLeft, FiChevronRight } from "react-icons/fi";
...
const DeleteLogsTab = () => {
  ...
  const fetchDeleteLogs = async () => {
    try {
      ...
      if (!token) {
        setLogs([]);
        toast({ title: "Authentication Error", description: "Please log in again.", status: "warning", duration: 5000, isClosable: true });
        return;
      }
      ...
      if (error.response && (error.response.status === 401 || error.response.status === 403)) {
        setLogs([]);
        toast({ title: "Authentication Error", description: "Session expired. Please log in again.", status: "warning", duration: 5000, isClosable: true });
      } else {
        toast({ title: "Error", description: "Failed to fetch delete logs.", status: "error", duration: 3000, isClosable: true });
      }
    } finally {
      setLoading(false);
    }
  ...
  const filteredLogs = logs.filter((log) => {
    const matchesSearch = searchTerm === "" ||
      ...
      log.originalCollection?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const confirmRestore = async () => {
    ...
    try {
      ...
      toast({ title: "Success", description: "Item restored successfully.", status: "success" });
      fetchDeleteLogs(); // Refresh list
    } catch (error) {
      console.error("Error restoring item:", error);
      toast({ title: "Error", description: error.response?.data?.message || "Failed to restore item.", status: "error" });
    }
    onRestoreClose();
    setLogToAction(null);
  ...
  const confirmPermanentDelete = async () => {
    ...
    try {
      ...
      toast({ title: "Success", description: "Item marked as permanently deleted.", status: "success" });
      fetchDeleteLogs(); // Refresh list
    } catch (error) {
      console.error("Error marking item as permanently deleted:", error);
      toast({ title: "Error", description: error.response?.data?.message || "Failed to permanently delete item.", status: "error" });
    }
    ...
    <Box>
          ...
          <Tbody>
            {loading ? (
              ...
              currentLogs.map((log) => {
                ...
                const canPermanentlyDelete = !log.isDeletedPermanently && !log.restoredAt; // Can't permanently delete restored items
                ...
                return (
                  <Tr key={log._id}>
                    ...
                    <Td whiteSpace="nowrap">{formatDateTime(log.restoredAt)}</Td>
                    <Td>
                      <HStack spacing={1}>
              ...
              <Tr><Td colSpan={10} textAlign="center">No delete logs found.</Td></Tr>
            )}
          </Tbody>
        </Table>
      </Box>
          ...
          <ModalBody>
            {selectedLog && (
              <Box>
                ...
                <Text mb={2} fontSize="sm">{selectedLog._id}</Text>

                <Text fontWeight="bold">Module:</Text>
                <Text mb={2}>{selectedLog.module}</Text>

                <Text fontWeight="bold">User:</Text>
                <Text mb={2}>{selectedLog.username} ({selectedLog.userRole})</Text>
                ...
                <Badge mb={2} colorScheme={getStatus(selectedLog).color}>{getStatus(selectedLog).label}</Badge>
                ...
                {typeof selectedLog.deletedData === 'object' && selectedLog.deletedData !== null ? (
                    <Box border="1px" borderColor="gray.200" borderRadius="md" p={3} mt={1} maxHeight="250px" overflowY="auto">
                    {Object.entries(selectedLog.deletedData).map(([key, value]) => (
                        <Flex key={key} mb={1} wrap="wrap">
                        <Text fontWeight="semibold" mr={2} minWidth="100px">{key}:</Text>
                    ...
                    <Text fontSize="sm" color="gray.500">No detailed data available.</Text>
                )}
      ...
      >
        <AlertDialogOverlay><AlertDialogContent>
          <AlertDialogHeader fontSize="lg" fontWeight="bold">Restore Item</AlertDialogHeader>
          <AlertDialogBody>
            Restore the selected {logToAction?.dataType || 'item'}?
            <br />
            This was originally deleted from {logToAction?.module || 'a module'}.
            <br />
            It will be added back to the '${logToAction?.originalCollection}' data.
          </AlertDialogBody>
          <AlertDialogFooter>
            <Button ref={cancelRef} onClick={onRestoreClose}>Cancel</Button>
            <Button colorScheme="green" onClick={confirmRestore} ml={3} isLoading={isRestoring}>Restore</Button>
          </AlertDialogFooter>
      ...
      >
        <AlertDialogOverlay><AlertDialogContent>
          <AlertDialogHeader fontSize="lg" fontWeight="bold" color="red.600">Permanently Delete Item</AlertDialogHeader>
          <AlertDialogBody>
            ...
            {logToAction && ` (Type: ${logToAction.dataType}, Module: ${logToAction.module})`}
            The original data will NOT be recoverable through this system after this action.
          </AlertDialogBody>
          <AlertDialogFooter>
            <Button ref={cancelRef} onClick={onPermanentDeleteClose}>Cancel</Button>
            <Button colorScheme="red" onClick={confirmPermanentDelete} ml={3} isLoading={isDeletingPermanently}>Delete Permanently</Button>
          </AlertDialogFooter>
        </AlertDialogContent></AlertDialogOverlay>
      </AlertDialog>
